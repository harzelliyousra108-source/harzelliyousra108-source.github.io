...
let countDown = new Date(`${config.birthdate} 01:00:00`).getTime();
x = setInterval(function() {
  ...
  for (let i = 0; i < opts.strings.length; ++i) {
    for (let j = 0; j < opts.strings[i].length; ++j) {
      const TEXT_OFFSET_Y = -80; // ⬆️ move text UP (try -60, -80, -100)
      letters.push(
  new Letter(
    opts.strings[i][j],
    j * opts.charSpacing +
...
const CAKE_DELAY = 4500; // ⏱️ milliseconds (4.5s)
const LOVE_TEXT_DELAY = 2500; // ⏱️ text appears first
